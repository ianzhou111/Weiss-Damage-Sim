  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://localhost:7060/api/attack/initialize-decks",
        {
        ...
        <div className="grid grid-cols-3 gap-4">
          {Object.keys(decks).map((deck) => (
            <div key={deck} className="bg-gray-700 p-4 rounded-lg shadow-md flex flex-col items-center">
              <h3 className="text-xl font-semibold text-center mb-4 w-full">{deck.replace(/([A-Z])/g, " $1").trim()}</h3>
              <div className="grid grid-rows-6 gap-3 w-full">
                {Object.keys(decks[deck]).map((key) => (
                  <div key={key} className="flex flex-col items-center w-full">
                    <span className="font-semibold mb-1 text-center w-full">{key.replace(/([A-Z])/g, " $1").trim()}</span>
                    <input
                      type="number"
                      name={key}
                ...
                <div className="font-bold mt-2">Total Cards: {calculateTotalCards(decks[deck])}</div>
              </div>
            </div>
          ))}
        </div>
